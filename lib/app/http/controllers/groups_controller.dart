    ...
    try {
      final rows = await connection!.select(
        ...
        SELECT
          ...
          CASE WHEN MAX(CASE WHEN gm_me.user_id IS NOT NULL THEN 1 ELSE 0 END) = 1 THEN 1 ELSE 0 END AS is_member
        FROM groups g
        LEFT JOIN group_members gm
          ON TRIM(gm.group_id) = TRIM(g.id::text) AND gm.status = 'active'
        LEFT JOIN group_members gm_me
          ON TRIM(gm_me.group_id) = TRIM(g.id::text)
  ...
  Future<Response> joinGroup(Request request, [dynamic _]) async {
    ...
    try {
      final groupRows = await connection!.select(
        '''
        SELECT g.id, g.name, g.required_level, g.conversation_id, COALESCE(u.bling_score, 0) AS my_level
        FROM groups g
      ...
      final existing = await connection!.select(
        ...
        WHERE TRIM(group_id) = TRIM(\$1) AND TRIM(user_id) = TRIM(\$2) AND status = 'active'
        LIMIT 1
        ''',
        [groupId, me],
      );
